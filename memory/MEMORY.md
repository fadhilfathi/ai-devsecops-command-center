...
- [s2-5-sbom-wire-format-drift-fix.md](./s2-5-sbom-wire-format-drift-fix.md) — S2.5 hotfix on `agents/roles/security/sbom-generator/`: 4 O-3.7 wire-format drift items + 3 missing required fields fixed; 5 helper funcs in `agent.py`; 2 new contract tests (compliance + drift regression); SS-07 test inventory 8→11 cases; canary rollback flags (`--rollback`, `--abort`); Syft SSRF warnings counter wired for Sprint 3.1 F1 burn-rate alert
