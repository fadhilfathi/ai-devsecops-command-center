    ...
    >
      ...
      <Card>
        ...
        <Card.Body className="flex flex-col items-center justify-center text-center">
          ...
          <div className="aion-mono mt-1 text-[11px] uppercase tracking-wider">out of 100</div>
...
function SubMetricTile({ metric }: { metric: SecurityScoreSubMetric }) {
  ...
  const isImproving =
    typeof metric.delta === 'number' && (inverted ? metric.delta < 0 : metric.delta > 0);
  const TrendIcon =
    metric.delta == null
      ? Minus
      : isImproving
        ...
        : TrendingUp;
  const trendColor =
    metric.delta == null ? 'text-muted' : isImproving ? 'text-success' : 'text-danger';
  ...
  return (
    ...
    >
      <div className="text-[11px] uppercase tracking-wider text-muted">{metric.label}</div>
      <div className="mt-1 flex items-baseline justify-between gap-2">
        <div className="text-2xl font-semibold tabular-nums text-text">{formatted}</div>
        <div className="h-7 w-24 shrink-0">
          <Sparkline data={metric.sparkline} ariaLabel={`${metric.label} trend`} />
        </div>
      </div>
