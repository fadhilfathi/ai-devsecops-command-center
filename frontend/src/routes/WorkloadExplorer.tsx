...
export function WorkloadExplorer() {
  ...
  const columns: Column<Workload & { key: string }>[] = [
    ...
    { key: 'namespace', header: 'Namespace', cell: (w) => w.namespace },
    {
      key: 'replicas',
      header: 'Replicas',
      cell: (w) => `${w.replicas.ready}/${w.replicas.desired}`,
    },
    {
      key: 'health',
      header: 'Health',
    ...
    },
    {
      ...
      header: 'CPU req/lim',
      cell: (w) =>
        `${fmtCpu(w.resources.cpuRequestsMillicores)} / ${fmtCpu(w.resources.cpuLimitsMillicores)}`,
    },
    {
      key: 'mem',
      header: 'Memory req/lim',
      cell: (w) =>
        `${fmtBytes(w.resources.memoryRequestsBytes)} / ${fmtBytes(w.resources.memoryLimitsBytes)}`,
    },
  ];

  const healthy = workloads.items.filter((w) => w.health === 'healthy').length;
  const unhealthy = workloads.items.filter(
    (w) => w.health === 'unhealthy' || w.health === 'degraded',
  ).length;
