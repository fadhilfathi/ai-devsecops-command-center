    ...
    <div className="space-y-6">
      ...
      <KpiGrid>
        ...
        <KpiTile
          ...
          hint={
            costItem
              ? `save $${fmtNumber(Math.round(costItem.potentialMonthlySavingsUsd))}/mo`
              : '—'
          }
        />
        <KpiTile
        ...
        {clusters.items.map((c) => (
          <Card key={c.id}>
            <Card.Header
              ...
              actions={
                <Badge variant="severity" severity={c.phase === 'active' ? 'info' : 'medium'}>
                  {titleCase(c.phase)}
                </Badge>
              }
            />
            <Card.Body>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <Stat
                  ...
                  label="Nodes"
                  value={`${c.readyNodes}/${c.nodeCount}`}
                />
                <Stat
                  icon={<Boxes className="h-4 w-4" />}
                  label="CPU"
                  value={`${c.totalCpuCores} cores`}
                />
                <Stat
                  icon={<GitBranch className="h-4 w-4" />}
                  label="Memory"
                  value={`${Math.round(c.totalMemoryBytes / 1024 ** 3)} GiB`}
                />
                <Stat
                  icon={<AlertTriangle className="h-4 w-4" />}
                  label="Env"
                  value={titleCase(c.environment)}
                />
              </div>
            </Card.Body>
          </Card>
            ...
            <Card.Body>
              {costItem ? (
                <div className="grid grid-cols-3 gap-4">
                  <KpiTile
                    label="Current"
                    value={`$${fmtNumber(Math.round(costItem.currentMonthlyUsd))}`}
                  />
                  <KpiTile
                    label="Recommended"
                    value={`$${fmtNumber(Math.round(costItem.recommendedMonthlyUsd))}`}
                  />
                  <KpiTile
                    label="Savings"
                    value={`$${fmtNumber(Math.round(costItem.potentialMonthlySavingsUsd))}`}
                  />
                </div>
              ) : (
