...
export function RuntimeSecurity() {
  ...
  const filtered =
    filter === 'all' ? risksData.items : risksData.items.filter((r) => r.level === filter);
  ...
  const columns: Column<RuntimeRisk & { key: string }>[] = [
    ...
    },
    {
      key: 'remediation',
      header: 'Remediation',
      cell: (r) => <span className="text-xs text-aion-muted">{r.remediation}</span>,
    },
  ];
                ...
                <li key={rec.id} className="rounded-md border border-aion-border p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-aion-muted">
                      ({fmtNumber(rec.affectedCount)} affected)
                    </span>
                  </div>
