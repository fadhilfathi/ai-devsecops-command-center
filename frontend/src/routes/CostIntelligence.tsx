...
export function CostIntelligence() {
  ...
  const workloadColumns: Column<WorkloadCost & { key: string }>[] = [
    ...
    {
      key: 'req',
      header: 'Requests (cpu/mem)',
      cell: (w) => `${fmtCpu(w.requests.cpuMillicores)} / ${fmtBytes(w.requests.memoryBytes)}`,
    },
    ...
    { key: 'rec', header: 'Recommended $/mo', cell: (w) => fmtUsd(w.recommendedMonthlyUsd) },
    {
      key: 'sav',
      header: 'Savings $/mo',
      cell: (w) => <span className="text-aion-ok">{fmtUsd(w.potentialMonthlySavingsUsd)}</span>,
    },
  ];
  ...
  const findingColumns: Column<CostFinding & { key: string }>[] = [
    ...
    },
    {
      key: 'subject',
      header: 'Workload',
      cell: (f) => `${f.namespace ?? '—'}/${f.workloadName ?? '—'}`,
    },
  ...
  const recColumns: Column<CostRecommendation & { key: string }>[] = [
    ...
    { key: 'title', header: 'Recommendation', cell: (r) => r.title },
    {
      key: 'detail',
      header: 'Detail',
      cell: (r) => <span className="text-xs text-aion-muted">{r.detail}</span>,
    },
      ...
      {a && (
        <KpiGrid>
          ...
          <KpiTile
            ...
            value={fmtUsd(a.potentialMonthlySavingsUsd)}
            hint={`${Math.round((a.potentialMonthlySavingsUsd / Math.max(1, a.currentMonthlyUsd)) * 100)}% reduction`}
          />
