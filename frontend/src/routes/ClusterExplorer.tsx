  ...
  { key: 'architecture', header: 'Arch', cell: (n) => n.architecture ?? '—' },
  {
    ...
    cell: (n) => (
      <div className="flex flex-wrap gap-1">
        ...
        {n.conditions.map((c) => (
          <Badge key={c} variant={c === 'ready' ? 'ok' : 'warn'}>
            {c}
          </Badge>
        ))}
      ...
      {cluster ? (
        <>
          <KpiGrid>
            <KpiTile
              label="Provider"
              value={titleCase(cluster.provider)}
              hint={cluster.region ?? '—'}
            />
            <KpiTile label="K8s version" value={cluster.k8sVersion ?? '—'} />
            <KpiTile
              label="Nodes"
              value={`${cluster.readyNodes}/${cluster.nodeCount}`}
              hint="ready / total"
            />
            ...
            <KpiTile
              label="Health"
              value={
                clusterHealth ? `${clusterHealth.score.score} (${clusterHealth.score.band})` : '—'
              }
            />
          </KpiGrid>
          ...
          <Card>
            <Card.Header
              title={`Nodes — ${cluster.name}`}
              subtitle="Ready nodes, conditions, and unschedulable state"
            />
            <Card.Body>
              <DataTable
          ...
          <Card>
            <Card.Header
              title="Namespaces"
              subtitle={`${namespaces.items.length} namespace(s) on this cluster`}
            />
            <Card.Body>
              <DataTable
                ...
                columns={[
                  ...
                  },
                  {
                    key: 'workloadCount',
                    header: 'Workloads',
                    cell: (n) => fmtNumber(n.workloadCount),
                  },
                  ...
                  { key: 'pendingPods', header: 'Pending', cell: (n) => fmtNumber(n.pendingPods) },
                  {
                    ...
                    header: 'Failed',
                    cell: (n) => (
                      <span className={n.failedPods > 0 ? 'text-severity-high' : ''}>
                        {fmtNumber(n.failedPods)}
                      </span>
                    ),
                  },
                  {
                    key: 'restartsLast1h',
                    header: 'Restarts 1h',
                    cell: (n) => fmtNumber(n.restartsLast1h),
                  },
                ]}
                rowKey={(n) => n.id}
              />
        ...
        <Card>
          <Card.Body>
            <div className="text-sm text-aion-muted">No clusters onboarded yet.</div>
          </Card.Body>
        </Card>
      )}
    </div>
