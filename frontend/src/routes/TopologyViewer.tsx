    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                  ...
                  return (
                    <g key={n.id} style={{ cursor: 'pointer' }} onClick={() => setSelectedId(n.id)}>
                      <circle
                        ...
                        strokeWidth={2}
                      />
                      <text x={p.x} y={p.y + 30} fontSize={11} fill="#e2e8f0" textAnchor="middle">
                        {n.label}
                      </text>
                      <text x={p.x} y={p.y + 44} fontSize={9} fill="#94a3b8" textAnchor="middle">
                        {n.kind}
                      </text>
                    </g>
                  );
        ...
        <Card>
          <Card.Header
            title="Selection"
            subtitle={selected ? selected.label : 'Click a node to inspect it'}
          />
          <Card.Body>
            {selected ? (
              <div className="space-y-2 text-xs">
                <div>
                  <span className="text-aion-muted">Kind:</span> {selected.kind}
                </div>
                <div>
                  <span className="text-aion-muted">Namespace:</span> {selected.namespace ?? '—'}
                </div>
                <div>
                  <span className="text-aion-muted">Cluster:</span> {selected.clusterName ?? '—'}
                </div>
                <div>
                  <span className="text-aion-muted">Tags:</span> {selected.tags.join(', ') || '—'}
                </div>
                <div>
                  ...
                  <ul className="mt-2 space-y-1">
                    {selectedEdges.map((e) => (
                      <li key={e.id} className="rounded border border-aion-border p-1 text-[11px]">
                        <Badge variant="neutral">{e.kind}</Badge> {e.source} → {e.target}
