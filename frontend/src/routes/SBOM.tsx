  ...
  return (
    <div>
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 lg:grid-cols-[280px_1fr]">
        ...
        <Card>
          <Card.Header title="Available SBOMs" subtitle="Per-asset generated inventories" />
                  ...
                  <li key={s.id}>
                    <button
                      ...
                      className={`flex w-full items-center justify-between gap-2 px-4 py-2.5 text-left text-sm transition-colors ${
                        on ? 'bg-accent/10 text-accent' : 'hover:bg-surface-2 text-text'
                      }`}
                    >
                      <span className="flex items-center gap-2">
                        ...
                        <span>
                          ...
                          <span className="aion-mono block text-[10px] text-muted">{s.id}</span>
                        </span>
                      </span>
        ...
        <div>
          {data && (
            <div className="mb-3 flex items-center gap-2 text-sm text-muted">
              ...
              <span>
                Listing <span className="text-text">{data.length}</span> components in the
                lightweight index
              </span>
            </div>
          )}
          ...
          <div className="mt-4 flex items-center gap-2">
            <Button
              size="sm"
              variant="secondary"
              onClick={() => window.location.assign(`/graph/${encodeURIComponent(activeId)}`)}
            >
