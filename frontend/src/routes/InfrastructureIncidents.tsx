...
export function InfrastructureIncidents() {
  ...
  const columns: Column<Incident & { key: string }>[] = [
    ...
    },
    {
      ...
      cell: (i) => (
        <Badge
          variant={i.status === 'resolved' ? 'ok' : i.status === 'investigating' ? 'warn' : 'info'}
        >
          {titleCase(i.status)}
        </Badge>
      ),
  ...
  return (
    <div className="space-y-6">
      ...
      <KpiGrid>
        ...
        <KpiTile
          label="Critical"
          value={fmtNumber(incidents.filter((i) => i.severity === 'critical').length)}
        />
        <KpiTile
          label="Open"
          value={fmtNumber(
        ...
        <KpiTile
          label="Resolved"
          value={fmtNumber(incidents.filter((i) => i.status === 'resolved').length)}
        />
      </KpiGrid>
