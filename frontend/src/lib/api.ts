...
export const api = {
  ...
  // ---- Vulnerabilities ---------------------------------------------------
  vulnerabilities: () => get<Vulnerability[]>('/vulnerabilities', mockVulnerabilities),
  ...
  // ---- Compliance --------------------------------------------------------
  compliance: () => get<ComplianceControl[]>('/compliance/controls', mockCompliance),
  ...
  /** Onboarded clusters. */
  kubernetesClusters: () =>
    get<{ items: Cluster[]; total: number }>('/kubernetes/clusters', {
      items: mockClusters,
      total: mockClusters.length,
    }),
  kubernetesNamespaces: (clusterId?: string) =>
    get<{ items: Namespace[]; total: number }>(
      clusterId ? `/kubernetes/namespaces?clusterId=${clusterId}` : '/kubernetes/namespaces',
  ...
  kubernetesServices: (clusterId?: string) =>
    get<{ items: K8sService[]; total: number }>(
      clusterId ? `/kubernetes/services?clusterId=${clusterId}` : '/kubernetes/services',
      { items: mockServices, total: mockServices.length },
    ),
  kubernetesDeployments: (clusterId?: string) =>
    get<{ items: Deployment[]; total: number }>(
      clusterId ? `/kubernetes/deployments?clusterId=${clusterId}` : '/kubernetes/deployments',
      { items: mockDeployments, total: mockDeployments.length },
    ),
  kubernetesIngresses: (clusterId?: string) =>
    get<{ items: Ingress[]; total: number }>(
      clusterId ? `/kubernetes/ingresses?clusterId=${clusterId}` : '/kubernetes/ingresses',
      { items: mockIngresses, total: mockIngresses.length },
    ),

  // ---- K8s health ---------------------------------------------------
  healthClusters: () =>
    get<{ items: InfrastructureHealth[]; total: number }>('/k8s-health/clusters', {
      items: mockHealth.filter((h) => h.scope === 'cluster'),
      total: mockHealth.filter((h) => h.scope === 'cluster').length,
    }),
  healthNamespaces: () =>
    get<{ items: InfrastructureHealth[]; total: number }>('/k8s-health/namespaces', {
      items: [],
      total: 0,
    }),
  healthWorkloads: () =>
    get<{ items: InfrastructureHealth[]; total: number }>('/k8s-health/workloads', {
      items: [],
      total: 0,
    }),
  healthPods: () =>
    get<{ items: InfrastructureHealth[]; total: number }>('/k8s-health/pods', {
      items: [],
      total: 0,
    }),
  ...
  healthIssues: () =>
    get<{ items: InfrastructureHealth['issues']; total: number }>('/k8s-health/issues', {
