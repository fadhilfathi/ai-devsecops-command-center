...
function makePods(tenantId: string, opts: ListOptions): Pod[] {
  const c = (
    ...
    ready: boolean,
    restarts: number,
    term: 'crash_loop_back_off' | 'image_pull_back_off' | 'oom_killed' | 'unknown' = 'unknown',
  ): Container => ({
    name,
    image,
  ...
  return [
    {
      ...
      containers: [c('app', 'ghcr.io/example/payments-api:1.42.0', true, 0, 'unknown')],
    ...
    },
    {
      ...
      clusterName: 'fixture',
      namespace: opts.namespace ?? 'default',
      name: 'payments-api-7d4f8b-abcd2',
      phase: phase(0),
      ownerKind: 'Deployment',
      ownerName: 'payments-api',
      serviceAccount: 'default',
      containers: [
        c('app', 'ghcr.io/example/payments-api:1.42.0', false, 8, 'crash_loop_back_off'),
      ],
      conditions: [
        {
          type: 'ready',
          status: 'false',
          message: 'containers with unready state: app',
          lastTransitionTime: NOW(),
        },
      ],
      restarts: 8,
      lastTerminationReason: 'crash_loop_back_off',
      labels: { app: 'payments-api' },
      annotations: {},
      createdAt: NOW(),
      updatedAt: NOW(),
  ...
  return [
    {
      ...
      endpoints: [],
      fqdn: 'payments-api.default.svc.cluster.local',
