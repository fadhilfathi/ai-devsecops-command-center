...
function buildSyntheticHealth(
  ...
  return clusters.map((c) => {
    ...
    const crashed = clusterPods.filter(
      (p) => p.lastTerminationReason === 'crash_loop_back_off',
    ).length;
...
function buildSyntheticRuntime(
  ...
  clusters: Cluster[],
  pods: Pod[],
): RuntimeSecurityReport | undefined {
  if (clusters.length === 0) return undefined;
  const cluster = clusters[0]!;
  const findings = pods
    .filter((p) => p.clusterId === cluster.id && p.lastTerminationReason !== 'unknown')
    .map((p) => ({
      id: p.id,
      tenantId,
      ...
      subject: 'pod' as const,
      subjectKind: 'Pod',
      subjectName: p.name,
      ruleId: 'AICC-RT-OBS',
      ruleName: 'Observed termination',
      category: 'unsafe_security_context' as const,
      level:
        p.lastTerminationReason === 'crash_loop_back_off'
          ? ('critical' as const)
          : ('high' as const),
      severity:
        p.lastTerminationReason === 'crash_loop_back_off'
  ...
  return {
    id: `rr-synth-${cluster.id}`,
    tenantId,
    clusterId: cluster.id,
    windowStart: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(),
    windowEnd: new Date().toISOString(),
    riskLevel: findings.some((f) => f.level === 'critical')
      ? 'critical'
      : findings.length > 0
        ? 'high'
        : 'low',
    score: Math.max(
      0,
      100 -
        8 * findings.filter((f) => f.level === 'critical').length -
        4 * findings.filter((f) => f.level === 'high').length,
    ),
    counts: {
    ...
    recommendations:
      findings.length > 0
        ? [
            {
              title: 'Investigate recent terminations',
              detail: 'Restart the affected workloads after inspecting logs.',
              level: 'high' as const,
...
function buildSyntheticCost(
  ...
  return {
    ...
    workloads: workloads.map((w) => ({
      ...
      kind: w.kind,
      currentMonthlyUsd: Math.round(
        (w.resources.cpuRequestsMillicores / 1000) * 0.041 * 24 * 30 +
          (w.resources.memoryRequestsBytes / 1024 ** 3) * 0.005 * 24 * 30,
      ),
      recommendedMonthlyUsd: Math.round(
        ((w.resources.cpuRequestsMillicores / 1000) * 0.041 * 24 * 30 +
          (w.resources.memoryRequestsBytes / 1024 ** 3) * 0.005 * 24 * 30) *
          0.6,
      ),
      potentialMonthlySavingsUsd: Math.round(
        ((w.resources.cpuRequestsMillicores / 1000) * 0.041 * 24 * 30 +
          (w.resources.memoryRequestsBytes / 1024 ** 3) * 0.005 * 24 * 30) *
          0.4,
      ),
...
function buildSyntheticTopology(
  tenantId: string,
  cluster: Cluster,
  workloads: Workload[],
  services: Service[],
  ...
  const nodes = [
    ...
    ...services.map((s) => ({
      ...
      tags: Object.entries(s.selector).map(([k, v]) => `${k}=${v}`),
      metadata: {},
    })),
    ...workloads.map((w) => ({
      id: w.id,
...
export function buildInventoryClient(deps: { logger: Logger }): InventoryClient {
  ...
  return {
    async fetch(tenantId, clusterId) {
      ...
      const costAnalysis = target[0]
        ? buildSyntheticCost(tenantId, target[0], workloads)
        : undefined;
      const topology = target[0]
        ? buildSyntheticTopology(tenantId, target[0], workloads, services, ingresses)
        : undefined;
      return {
        clusters: target,
        namespaces,
