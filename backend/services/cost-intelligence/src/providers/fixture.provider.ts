/**
 * Fixture provider for cost-intelligence — provides a workload
 * mix that surfaces realistic findings (over-provisioned, under-
 * utilized, missing limits).
 */
...
function makeDeployments(tenantId: string, opts: ListOptions): Deployment[] {
  return [
    ...
    // missing requests
    {
      ...makeWorkload({
        tenantId, clusterId: opts.clusterId, namespace: opts.namespace ?? 'default',
        kind: 'deployment', name: 'experimental',
        image: 'example/experimental:0.0.1',
        desired: 1, ready: 1, updated: 1, available: 1,
        resources: { cpuReq: 0, cpuLim: 500, memReq: 0, memLim: 256 * 1024 * 1024 },
      }),
      strategy: 'rolling_update', rollingUpdate: { maxSurge: 1, maxUnavailable: 0 },
      currentReplicaSet: 'experimental-1', terminatingReplicas: 0,
      rollout: 'complete' as DeploymentRolloutStatus, paused: false,
    },
  ];
}
...
function makePods(_tenantId: string, opts: ListOptions): Pod[] {
  ...
  return [
    { id: uuid(), tenantId: _tenantId, clusterId: opts.clusterId, clusterName: 'fixture', namespace: opts.namespace ?? 'default', name: 'payments-api-7d4f8b-abcd1', phase: phase(1), node: 'ip-10-0-2-10', podIp: '10.42.0.10', ownerKind: 'Deployment', ownerName: 'payments-api', serviceAccount: 'payments-api', containers: [c('app', 'ghcr.io/example/payments-api:1.42.0', true, 0)], conditions: [{ type: 'ready', status: 'true', lastTransitionTime: NOW() }], restarts: 0, startedAt: NOW(), lastTerminationReason: 'unknown', labels: { app: 'payments-api' }, annotations: {}, createdAt: NOW(), updatedAt: NOW(), lastSyncedAt: NOW() },
  ];
}

function makeServices(_tenantId: string, opts: ListOptions): Service[] {
  return [{
...
function makeIngresses(_tenantId: string, opts: ListOptions): Ingress[] {
  return [{
    ...
    tls: [], labels: {}, createdAt: NOW(), updatedAt: NOW(), lastSyncedAt: NOW(),
  }];
}
