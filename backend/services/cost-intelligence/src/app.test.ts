...
test('GET /v1/cost/analysis without x-tenant-id is rejected', async () => {
  const server = await buildServer();
  const res = await server.inject({ method: 'GET', url: '/v1/cost/analysis' });
  expect(res.statusCode).toBe(400);
  await server.close();
});

test('GET /v1/cost/analysis with x-tenant-id returns cost analyses', async () => {
  const server = await buildServer();
  const res = await server.inject({
    method: 'GET',
    url: '/v1/cost/analysis',
...
test('GET /v1/cost/analysis without a token is 401 when dev bypass is off', async () => {
