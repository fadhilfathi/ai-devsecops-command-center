    ...
    conditions: [
      {
        ...
        message:
          args.available >= args.desired ? 'Minimum replicas available' : 'Below desired replicas',
...
function buildFixtureClusters(tenantId: string): Cluster[] {
  return [
    ...
    },
    {
      ...
      nodes: [
        {
          name: 'gke-staging-pool-1',
          roles: ['worker'],
          kubeletVersion: 'v1.28.9',
          architecture: 'amd64',
          conditions: ['ready'],
          unschedulable: false,
        },
        {
          name: 'gke-staging-pool-2',
          roles: ['worker'],
          kubeletVersion: 'v1.28.9',
          architecture: 'amd64',
...
function buildFixturePods(tenantId: string, opts: ListOptions): Pod[] {
  ...
  return [
    ...
    },
    {
      ...
      clusterName: 'cluster',
      namespace: opts.namespace ?? 'default',
      name: 'orders-api-9aa-efef',
      phase: phase(3),
      node: 'ip-10-0-2-11',
      podIp: '10.42.1.5',
      ownerKind: 'Deployment',
      ownerName: 'orders-api',
      serviceAccount: 'default',
      containers: containers('app', 'ghcr.io/example/orders-api:2.1.0', true, 2),
      conditions: [{ type: 'ready', status: 'true', lastTransitionTime: NOW() }],
      restarts: 2,
      startedAt: NOW(),
      lastTerminationReason: 'oom_killed',
...
function buildFixtureServices(tenantId: string, opts: ListOptions): Service[] {
  return [
    ...
    },
    {
      ...
      endpoints: [
        {
          podName: 'orders-api-9aa-efef',
          podIp: '10.42.1.5',
          nodeName: 'ip-10-0-2-11',
          ready: true,
        },
      ],
