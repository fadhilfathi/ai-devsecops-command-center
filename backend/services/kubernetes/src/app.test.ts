...
test('GET /v1/kubernetes/providers without a token is 401 when dev bypass is off', async () => {
...
test('GET /v1/kubernetes/clusters authenticates with a real bearer token', async () => {
  const prev = process.env.AUTH_DEV_BYPASS;
  process.env.AUTH_DEV_BYPASS = 'false';
  try {
    const server = await buildServer();
