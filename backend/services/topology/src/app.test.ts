...
test('GET /v1/topology/graphs without x-tenant-id is rejected', async () => {
  const server = await buildServer();
  const res = await server.inject({ method: 'GET', url: '/v1/topology/graphs' });
  expect(res.statusCode).toBe(400);
  await server.close();
});

test('GET /v1/topology/graphs with x-tenant-id returns topology graphs', async () => {
  const server = await buildServer();
  const res = await server.inject({
    method: 'GET',
    url: '/v1/topology/graphs',
...
test('GET /v1/topology/graphs without a token is 401 when dev bypass is off', async () => {
...
test('a token for tenant A plus a forged x-tenant-id header still only sees tenant A data', async () => {
  ...
  try {
    ...
    expect(res.statusCode).toBe(200);
    const body = res.json();
    expect(body.items.every((g: { tenantId: string }) => g.tenantId === 'tenant-a')).toBe(true);
    await server.close();
  } finally {
    if (prev === undefined) delete process.env.AUTH_DEV_BYPASS;
    else process.env.AUTH_DEV_BYPASS = prev;
