/**
 * Fixture provider (runtime-security copy). Shares the same
 * deterministic data as the k8s-health fixture provider; kept
 * as a local copy so each service is self-contained for Sprint
 * 4. The Sprint 5 refactor will consolidate these into the
 * `@aicc/kubernetes-service` HTTP client.
 */
...
function makeDeployments(tenantId: string, opts: ListOptions): Deployment[] {
  return [
    ...
    },
    {
      ...
      strategy: 'rolling_update',
      rollingUpdate: { maxSurge: 1, maxUnavailable: 0 },
      currentReplicaSet: 'legacy-tool-1',
      terminatingReplicas: 0,
      rollout: 'complete' as DeploymentRolloutStatus,
      paused: false,
    },
...
function makePods(tenantId: string, opts: ListOptions): Pod[] {
  // Build a small "interesting" set so the rule engine produces
  // a realistic mix of findings.
  const c = (name: string, image: string, opts2: Partial<Container> = {}): Container => ({
    name,
    image,
    state: 'running',
    ready: true,
