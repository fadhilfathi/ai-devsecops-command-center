...
test('GET /v1/runtime-security/risks without a token is 401 when dev bypass is off', async () => {
...
test('a token for tenant A plus a forged x-tenant-id header still only sees tenant A data', async () => {
  ...
  try {
    ...
    expect(res.statusCode).toBe(200);
    const body = res.json();
    expect(body.items.every((r: { tenantId: string }) => r.tenantId === 'tenant-a')).toBe(true);
    await server.close();
  } finally {
    if (prev === undefined) delete process.env.AUTH_DEV_BYPASS;
    else process.env.AUTH_DEV_BYPASS = prev;
