/**
 * Runtime Security Service — entry point.
 */
...
import { buildRuntimeSecurityRoutes } from './routes/runtime-security.js';
import { buildRuntimeSecurityEngine } from './engine/runtime-security.engine.js';
import { buildInventoryClient } from './inventory/client.js';

const SERVICE_NAME = 'runtime-security-service';
const SERVICE_VERSION = '0.1.0';

export interface RuntimeSecurityServiceDeps {
  bus: EventBus;
  logger: Logger;
}

export async function buildServer(deps?: Partial<RuntimeSecurityServiceDeps>): Promise<FastifyInstance> {
  ...
  await server.register(buildRuntimeSecurityRoutes, { logger, inventory, engine, bus });
