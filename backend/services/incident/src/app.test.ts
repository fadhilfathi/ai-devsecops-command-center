...
test('GET /v1/incidents returns the incident list', async () => {
  const server = await buildServer();
...
test('GET /v1/incidents without a token is 401 when dev bypass is off', async () => {
  const prev = process.env.AUTH_DEV_BYPASS;
  process.env.AUTH_DEV_BYPASS = 'false';
  try {
    const server = await buildServer();
    const res = await server.inject({ method: 'GET', url: '/v1/incidents' });
...
test('a token for tenant A plus a forged x-tenant-id header still only sees tenant A data', async () => {
  ...
  try {
    ...
    await server.inject({
      ...
      payload: { title: 'Tenant A incident', description: 'seeded', severity: 'high' },
    });
    const res = await server.inject({
      method: 'GET',
      url: '/v1/incidents',
