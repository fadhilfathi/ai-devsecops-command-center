/**
 * Infrastructure Inventory Service — entry point.
 */
...
import { buildInventoryRoutes } from './routes/inventory.js';
import { buildGraphRoutes } from './routes/graph.js';
import { buildInventoryEngine } from './engine/inventory.engine.js';
...
export async function buildServer(deps?: Partial<InventoryServiceDeps>): Promise<FastifyInstance> {
  ...
  await server.register(buildInventoryRoutes, { logger, inventory, engine, bus });
  await server.register(buildGraphRoutes, { logger, inventory, engine, bus });
